...
def prediction_model_pipeline(DF, API_KEY, GPT_MODEL,openai_api_model_type,openai_api_base_url):
    st.divider()
    st.subheader('数据概述')
    ...
    if not st.session_state.target_selected:
        ...
        with st.spinner("AI is analyzing the data..."):
            ...
            st.session_state.target_Y = decide_target_attribute(attributes_for_target, types_info_for_target, head_info_for_target, GPT_MODEL, API_KEY,openai_api_model_type,openai_api_base_url)

        if st.session_state.target_Y != -1:
            selected_Y = st.session_state.target_Y
            st.success("AI已选择目标属性！")
            st.write(f'所选目标属性： :green[**{selected_Y}**]')
            st.session_state.target_selected = True
        else:
            st.info("AI无法从数据中确定目标变量。请选择目标变量")
        ...
        if st.session_state.target_Y != -1:
            st.success("AI已选择目标属性！")
        st.write(f"所选目标属性： :green[**{st.session_state.selected_Y}**]")

    if st.session_state.target_selected:

        # Data Imputation
        st.subheader('处理和输入缺失值')
        ...
        if 'filled_df' not in st.session_state:
            if st.session_state.contain_null:
                with st.status("Processing **missing values** in the data...", expanded=True) as status:
                    ...
                    fill_result_dict = decide_fill_null(attributes, types_info, description_info, GPT_MODEL, API_KEY,openai_api_model_type,openai_api_base_url)
                    ...
                    DF = filled_df
                    status.update(label='缺失值处理已完成！', state="complete", expanded=False)
                st.download_button(
        ...
        # Data Encoding
        st.subheader("进行数据预处理编码")
        st.caption("出于处理时间的考虑，TF-IDF等NLP特征尚未包含在当前的管道中，长文本属性可能会被删除。")
        ...
        if 'encoded_df' not in st.session_state:
            if not st.session_state.all_numeric:
                with st.status("Encoding non-numeric data using **numeric mapping** and **one-hot**...", expanded=True) as status:
                    ...
                    st.write("Large language model analysis...")
                    encode_result_dict = decide_encode_type(non_numeric_attributes, non_numeric_head, GPT_MODEL, API_KEY,openai_api_model_type,openai_api_base_url)
                    ...
                    DF = encoded_df
                    status.update(label='Data encoding 完成!', state="complete", expanded=False)
                st.download_button(
                    label="下载已经预处理编码的数据",
                ...
                st.success("所有列都是数字。处理已跳过。")
        else:
            st.success("使用one-hot编码完成数据编码")
            if not st.session_state.all_numeric:
                st.download_button(
                    label="下载已经预处理编码的数据",
                    data=st.session_state.encoded_df.to_csv(index=False).encode('utf-8'),
        ...
        if 'test_percentage' not in st.session_state:
            with st.spinner("Deciding testing set percentage based on data..."):
                st.session_state.test_percentage = int(decide_test_ratio(st.session_state.df_pca.shape, GPT_MODEL, API_KEY,openai_api_model_type,openai_api_base_url) * 100)

        splitting_column, balance_column = st.columns(2)
        with splitting_column:
            st.subheader('数据拆分')
            st.caption('AI推荐的模型测试百分比')
            st.slider('测试集百分比', 1, 25, st.session_state.test_percentage, key='test_percentage', disabled=st.session_state['start_training'])
        
        with balance_column:
            st.metric(label="测试数据", value=f"{st.session_state.test_percentage}%", delta=None)
            st.toggle('类平衡', value=st.session_state.balance_data, key='to_perform_balance', on_change=update_balance_data, disabled=st.session_state['start_training'])
            st.caption('处理不平衡数据集和提高机器学习模型性能的策略。')
            st.caption('AI将选择最合适的方法来平衡数据。')
        
        st.button("开始训练", on_click=start_training_model, type="primary", disabled=st.session_state['start_training'])
            ...
            with st.container():
                ...
                if st.session_state.balance_data and "balance_method" not in st.session_state:
                    with st.spinner("AI is deciding the balance strategy for the data..."):
                        ...
                        st.session_state.balance_method = int(decide_balance(shape_info_balance, description_info_balance, balance_info_balance, GPT_MODEL, API_KEY,openai_api_model_type,openai_api_base_url))
                ...
                if not st.session_state["decided_model"]:
                    with st.spinner("Deciding models based on data..."):
                        ...
                        model_dict = decide_model(shape_info, head_info, nunique_info, description_info, GPT_MODEL, API_KEY,openai_api_model_type,openai_api_base_url)
                ...
                if st.session_state["all_set"]:
                    ...
                    with download_col1:
                        st.download_button(label="下载模型", data=st.session_state.downloadable_model1, file_name=f"{st.session_state.model1_name}.joblib", mime="application/octet-stream")
                    with download_col2:
                        st.download_button(label="下载模型", data=st.session_state.downloadable_model2, file_name=f"{st.session_state.model2_name}.joblib", mime="application/octet-stream")
                    with download_col3:
                        st.download_button(label="下载模型", data=st.session_state.downloadable_model3, file_name=f"{st.session_state.model3_name}.joblib", mime="application/octet-stream")
