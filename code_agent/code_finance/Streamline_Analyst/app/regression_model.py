...
def regression_model_pipeline(DF, API_KEY, GPT_MODEL,openai_api_model_type,openai_api_base_url):
    st.divider()
    st.subheader('数据概述')
    ...
    if not st.session_state.target_selected:
        ...
        if st.session_state.target_Y != -1:
            ...
            st.write(f'已选择目标属性: :green[**{selected_Y}**]')
    ...
    if st.session_state.target_selected:
        ...
        with balance_column:
            ...
            st.toggle('类平衡', value=False, key='to_perform_balance', disabled=True)
            st.caption('类平衡不适用于回归模型。')
            ...
            with st.container():
                ...
                if not st.session_state["decided_model"]:
                    with st.spinner("Deciding models based on data..."):
                        ...
                        model_dict = decide_regression_model(shape_info, description_info, st.session_state.selected_Y, GPT_MODEL, API_KEY,openai_api_model_type,openai_api_base_url)
