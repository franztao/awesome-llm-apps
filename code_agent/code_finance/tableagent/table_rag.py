...
#### 适用场景
  ...
  - 诉讼记录统计分析""")
    ...
    st.markdown(
        """
        上传CSV、XLSX或JSON文件。该应用程序将摄取数据，构建数据库，并允许您提出基于SQL的问题。
        """
    )
    ...
    st.sidebar.header("可解释性选项")
    show_sql_debug = st.sidebar.checkbox("显示SQL查询", value=False)
    query_explainability_toggle = st.sidebar.checkbox("生成查询说明", value=False)

    st.sidebar.header("数据字典")
    data_dictionary_toggle = st.sidebar.checkbox("生成和使用数据字典", value=False)

    st.sidebar.header("调试选项")
    show_prompt_toggle = st.sidebar.checkbox("显示SQL生成提示", value=False)
    ...
    if data_dictionary_toggle:
        if st.button("刷新样本数据", key="refresh_sample"):
            with ro_engine.connect() as conn:
    ...
    def generate_sql_query(question: str) -> dict:
        ...
        #                 HumanMessage(content="请介绍 LangChain 的功能。")
        #             ]
        #             response = structured_llm(messages)
    ...
    if st.button("提交") and user_question:
        ...
        if query_explainability_toggle:
            with st.expander("SQL执行命令解释", expanded=True):
