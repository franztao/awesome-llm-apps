...
def extract_user_info_from_urls(urls: List[str], firecrawl_api_key: str) -> List[dict]:
    ...
    firecrawl_app = FirecrawlApp(api_key='fc-bd7f59397c2544e79a7236038b0ba662')
...
def main():
    ...
    st.markdown(
        """
        AI 潜在客户生成Agent可自动完成从 Quora 查找和筛选潜在客户的过程。它使用 Firecrawl 的搜索功能和新的 Extract 端点来识别相关用户资料、提取有价值的信息，并将其组织成 Google 表格中的结构化格式。此Agent可帮助销售和营销团队高效地建立有针对性的潜在客户列表，同时节省数小时的手动研究时间。
        ### 特征
