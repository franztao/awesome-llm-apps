...
def create_agents() -> tuple[Agent, Agent, Agent]:
    ...
    coding_agent = Agent(
        model=OpenAILike(id=st.session_state.openai_api_model_type, api_key=st.session_state.openai_api_key,base_url=st.session_state.openai_api_base_url,
            ...
            2. Write clean, efficient Python code_fin to solve it
            3. Include proper documentation and type hints
            4. The code_fin will be executed in an e2b sandbox environment
            Please ensure your code_fin is complete and handles edge cases appropriately.最后输出的内容必须是中文内容呈现，不要是英文""")
        ,
        markdown=True
    )
    ...
    execution_agent = Agent(
        model=OpenAILike(id=st.session_state.openai_api_model_type, api_key=st.session_state.openai_api_key,base_url=st.session_state.openai_api_base_url,
            system_prompt="""You are an expert at executing Python code_fin in sandbox environments.
            Your task is to:
            1. Take the provided Python code_fin
...
def process_image_with_gemini(vision_agent: Agent, image: Image) -> str:
    prompt = """Analyze this image and extract any coding problem or code_fin snippet shown. 
...
def execute_code_with_agent(execution_agent: Agent, code: str, sandbox: Sandbox) -> str:
    try:
        # Set timeout to 30 seconds for code_fin execution
        ...
        if execution.error:
            if "TimeoutException" in str(execution.error):
                return "⚠️ Execution Timeout: The code_fin took too long to execute (>30 seconds). Please optimize your solution or try a smaller input."
            
            error_prompt = f"""The code_fin execution resulted in an error:
        ...
        prompt = f"""Here is the code_fin execution result:
        Logs: {execution.logs}
        Files: {str(files)}
...
5. 检查任何错误消息或执行超时
    ...
    if st.button("生成并执行解决方案", type="primary"):
        ...
        if 'response' in locals():
            ...
            # Extract code_fin from markdown response
            ...
            if len(code_blocks) > 1:
                ...
                # Execute code_fin with execution agent
                with st.spinner("Executing code_fin..."):
